            ...
            content: (() => {
                ...
                contentLines = [
                    ...
                    ...contentLines.map(line => line
                        ...
                        .replace(`/memory/signal/`, `/${hash}/memory/signal/`)
                        .replace(`/memory/config/`, `/${hash}/memory/config/`)
                        .replace(`/memory/media/`, `/${hash}/memory/media/`)
                        .replace(`/memory/style/`, `/${hash}/memory/style/`)
                        .replace(`await fetch("https://localhost/`, `await fetch("https://api.dbdbdbdbdbgroup.com/`)
                        .replace(`ioClient("https://localhost"`, `ioClient("https://api.dbdbdbdbdbgroup.com"`)
                        .replace(`io("https://localhost"`, `io("https://api.dbdbdbdbdbgroup.com"`)
                        ...
                        .replace(`https://localhost/demo/api/memory/get`, `https://api.dbdbdbdbdbgroup.com/${hash}/api/memory/get`)
                        .replace(`https://localhost/demo/api/memory/set`, `https://api.dbdbdbdbdbgroup.com/${hash}/api/memory/set`)
        ...
        return {
            content: (() => {
                ...
                const outputLines = contentLines.map(line => {
                    ...
                    return line
                        .replace("/media/${name}.${suffix}?type=${type}", "https://api.dbdbdbdbdbgroup.com/media/${name}.${suffix}?type=${type}")
                        .replace("/media/${cells[0]}.${cells[2]}?${cells[1]}", "https://api.dbdbdbdbdbgroup.com/media/${cells[0]}.${cells[2]}?${cells[1]}")
                });
                return [
                    ...outputLines,
                ].join('\r\n')
