 ...
 * Description: Logic and operations for src\modules\util\uncombine.js.
 */
  ...
  try {
    ...
    for (const filePath in combinedData) {
      ...
      if (!fs.existsSync(dirPath)) {
        ...
        console.log(process.cwd(), `Created directory: ${dirPath}`);
      }

      fs.writeFileSync(absoluteFilePath, content, "utf-8");
      console.log(process.cwd(), `File written: ${absoluteFilePath}`);
    }

    console.log(process.cwd(), "Uncombining completed successfully.");
  } catch (error) {
