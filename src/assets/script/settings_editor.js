const initSettingsEditor = (editorEl) => {
    ...
    textEditorEl.addEventListener('keydown', () => (event.keyCode === 13) && textEditorElBlur());
    ...
    emojiWidgetTriggerEl.addEventListener('click', () => {
        if (emojiWidgetInstanceEl) {
            ...
            emojiEls.forEach(el => (emojiWidgetTriggerEl.innerText === el.innerText) && el.classList.add('active'));
